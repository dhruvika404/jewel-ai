...
type HeaderConfig = {
  ...
  visible?: boolean; // if false, hide common header for this page
...
const PageHeaderContext = createContext<HeaderContextType | undefined>(undefined);

export function PageHeaderProvider({ children }: { children: React.ReactNode }) {
  const [header, setHeaderState] = useState<HeaderConfig>({ visible: true });

  const setHeader = (config: HeaderConfig) => setHeaderState({ visible: true, ...(config ?? {}) });

...
export default function ExcelUpload() {
  ...
  const [fileBlobs, setFileBlobs] = useState<{ [key: string]: { blob: Blob, name: string, type: string } }>({})
  const [showDebug, setShowDebug] = useState(false)

  const testFormData = (file: File) => {
    const itemId = uploadItems.find(item => selectedFiles[item.id] === file)?.id
    const fileBlob = itemId ? fileBlobs[itemId] : null
    
    if (fileBlob) {
      // Test with preserved blob
      ...
      formData.append('file', preservedFile, preservedFile.name)

      toast.success('✅ FormData test with preserved content completed - check console')
    } else {
      // Fallback to original file
      const formData = new FormData()
      formData.append('file', file, file.name)
      toast.warning('⚠️ Using original file (no preserved content) - check console')
    }
  ...
  const handleFileSelect = (itemId: string, file: File | null) => {
    ...
    if (file) {
      ...
      // Read file content and store as blob to preserve it
      const reader = new FileReader()
      reader.onload = (e) => {
        ...
        if (arrayBuffer && arrayBuffer.byteLength > 0) {
          ...
          // Store both the original file and the blob
  ...
  const handleUpload = async (item: UploadItem) => {
    ...
    try {
      ...
      console.log("🚀 ~ handleUpload ~ preservedFile:", preservedFile)
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600">Upload your Excel files to import data into the system</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {uploadItems.map((item) => {
          ...
          return (
            <Card key={item.id} className={`transition-all duration-200 ${getStatusColor(status?.status || 'idle')}`}>
              ...
              <CardContent className="space-y-4">
                <div>
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      // Don't reset the input value to preserve file reference
                      if (selectedFile) {
                    ...
                    className="hidden"
                    id={`file-${item.id}`}
                    key={`${item.id}-${file?.name || 'empty'}`} // Unique key to prevent conflicts
                  />
                  <label
                ...
                >
                  {status?.status === 'uploading' ? 'Uploading...' : 
                   status?.status === 'success' ? 'Upload Again' : 'Upload'}
                </Button>

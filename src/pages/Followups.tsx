...
export default function Followups() {
  const { type: paramFollowupType } = useParams<{ type: string }>();
  ...
  // Loading effect for all filters
  useEffect(() => {
    if (!isInitialMount.current) {
      loadFollowupData();
    }
  ...
  // Main data loading effect - depends on all filters
  useEffect(() => {
    loadFollowupData();
  }, [
    followupType,
  ...
  // Effect to handle cleanup/reset ONLY when followupType (tab) changes
  useEffect(() => {
    if (isInitialMount.current) {
      isInitialMount.current = false;
      return;
    ...
    // Reset filters and clear data for the new tab
    setFollowups([]);
    setSalesPersonFilter("all");
    setClientFilter("all");
    setSearchTerm("");
  ...
  const getStatsForType = (type: FollowupType) => {
    switch (type) {
      case "new-order":
        return {
      ...
      default:
        return { pending: 0, taken: 0, last7: 0 };
    }
  };

  getStatsForType(followupType);

  const commonColumns: Column<FollowupRecord>[] = [
    {
      header: "Client Code",
      render: (fu) => (
        <span className="font-medium text-gray-900">{fu.userCode}</span>
      ),
    },
    {
      header: "Client Name",
    ...
    },
    {
      ...
      render: (fu) =>
        fu.salesExecutive ? (
          <div className="text-sm text-gray-900">{fu.salesExecutive}</div>
        ) : (
  ...
  const columns: Column<FollowupRecord>[] = ((): Column<FollowupRecord>[] => {
    switch (followupType) {
      case "new-order":
        return [
          ...commonColumns.slice(0, 2),
          {
            header: (
              ...
              >
                Last Order Date {getSortIcon("lastOrderDate")}
              </div>
            ),
            render: (fu: any) => (
          ...
          },
          {
            header: (
              ...
              >
                No Order Since {getSortIcon("noOrderSince")}
              </div>
            ),
            render: (fu: any) => (
              <div className="text-sm text-gray-900">
          ...
          commonColumns[2],
          {
            header: (
              ...
              >
                Next Followup {getSortIcon("nextFollowUpDate")}
              </div>
            ),
            render: (fu: any) => (
              <div className="text-sm text-gray-900">
              ...
              <div className="flex items-center justify-center gap-2">
                <Link
                  to={`${
                    ...
                    fu.originalData?.clientData?.uuid ||
                    fu.originalData?.clientId ||
                    fu.id
                  }`}
                >
                  <Button
        ...
        return [
          ...
          },
          {
            header: (
              ...
              >
                Pending Since {getSortIcon("pendingSince")}
              </div>
            ),
            render: (fu: any) => (
              <div className="text-sm text-gray-900">{fu.pendingSince} Days</div>
            ),
          },
          {
            header: (
              ...
              >
                Pending Pcs {getSortIcon("pendingPcs")}
              </div>
            ),
            render: (fu: any) => (
              <div className="text-sm text-gray-900">{fu.pendingPcs}</div>
            ),
          },
          {
            header: (
        ...
        return [
          ...
          },
          {
            header: (
              ...
              >
                Pending Since {getSortIcon("pendingSinceDays")}
              </div>
            ),
            render: (fu: any) => (
              <div className="text-sm text-gray-900">{fu.pendingSinceDays}</div>
            ),
          },
          {
            header: (

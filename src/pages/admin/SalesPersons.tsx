...
import { Upload, Loader2, KeyRound, Plus, Edit2, Users, Phone, Mail, FileText, Eye, EyeOff, Pencil, KeyRoundIcon } from 'lucide-react'
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (statsResponse.success !== false && statsResponse.data) {
          setStats({
              total: currentTotalItems || statsResponse.data.totalSalesPersons || 0,
              active: statsResponse.data.activeSalesPersons || 0,
              inactive: statsResponse.data.inactiveSalesPersons || 0
          })
      }
    } catch (error: any) {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedSalesPerson) {
        // Update
        const response = await salesPersonAPI.update(selectedSalesPerson.uuid, formData)
        if (response.success) {
    ...
    <div className="bg-gray-50 pb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Card className="p-4 border-none shadow-sm bg-white ring-1 ring-black/[0.05]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-medium text-gray-500">Inactive Users</p>
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-24 text-gray-500">
             ...
             <div className="flex flex-col items-center justify-center py-24 text-gray-500">
              ...
              <h3 className="text-lg font-medium text-gray-900">No sales persons found</h3>
              <p className="text-sm max-w-sm text-center mt-2">
                Get started by adding a new sales person or importing from an Excel file.
              </p>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-gray-50">
                    <TableHead className="font-medium text-gray-700 w-[100px]">Name</TableHead>
                    <TableHead className="font-medium text-gray-700 w-[100px]">Code</TableHead>
                    <TableHead className="font-medium text-gray-700 w-[200px]">Contact</TableHead>
                    <TableHead className="font-medium text-gray-700 w-[120px]">Status</TableHead>
                    <TableHead className="font-medium text-gray-700 text-center w-[120px]">Actions</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {salesPersons.map((sp) => (
                    <TableRow key={sp.uuid} className="hover:bg-gray-50">
                        ...
                        <div className="flex flex-col gap-1">
                          {sp.email && (
                            <div className="flex items-center gap-2 text-sm text-gray-600">
                              ...
                              <span className="truncate max-w-[180px]" title={sp.email}>{sp.email}</span>
                            </div>
                          )}
                          {sp.phone && (
                            <div className="flex items-center gap-2 text-sm text-gray-600">
                      ...
                      <TableCell className="align-top py-4">
                        <Badge 
                          ...
                          className={sp.isActive ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-emerald-200' : 'bg-gray-100 text-gray-700'}
                        >
      ...
      <Dialog open={showFormDialog} onOpenChange={setShowFormDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{selectedSalesPerson ? 'Edit Sales Person' : 'Add New Sales Person'}</DialogTitle>
            <DialogDescription>
              {selectedSalesPerson ? 'Update the details below' : 'Enter the details for the new team member'}
            </DialogDescription>
          </DialogHeader>
      ...
      {/* Set Password Dialog */}
      <Dialog open={showPasswordDialog} onOpenChange={(open) => !open && handleClosePasswordDialog()}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Set Login Password</DialogTitle>
            <DialogDescription>
              Set a new password for <span className="font-medium text-gray-900">{selectedSalesPerson?.name}</span>
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={showUploadDialog} onOpenChange={setShowUploadDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Upload an Excel file to bulk import sales persons. Supported formats: .xlsx, .xls
            </DialogDescription>
          </DialogHeader>
          
          <div className="border-2 border-dashed border-gray-200 rounded-xl p-8 text-center hover:bg-gray-50/50 transition-colors cursor-pointer"
            ...
            <p className="text-xs text-gray-500 mt-1">
              {uploadFile ? `${(uploadFile.size / 1024).toFixed(1)} KB` : 'or drag and drop here'}
            </p>
          </div>
